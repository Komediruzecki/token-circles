...
class LoansRepository extends BaseRepository {
  list(profileId) {
    return this.all('SELECT * FROM loans WHERE profile_id = ? ORDER BY created_at DESC', profileId);
  }
  ...
  getPrepayments(loanId) {
    return this.all('SELECT * FROM loan_prepayments WHERE loan_id = ? ORDER BY month', loanId);
  }
