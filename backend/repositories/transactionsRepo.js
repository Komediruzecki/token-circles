...
class TransactionsRepository extends BaseRepository {
  list(profileId, filters = {}) {
    let sql = 'SELECT t.*, c.name as category_name, c.color as category_color FROM transactions t LEFT JOIN categories c ON t.category_id = c.id WHERE t.profile_id = ?'
    ...
    if (filters.search)   { sql += ' AND (t.description LIKE ? OR t.notes LIKE ? OR t.beneficiary LIKE ? OR t.payor LIKE ?)'
      const q = `%${filters.search}%`; params.push(q, q, q, q) }
    if (filters.category_id) { sql += ' AND t.category_id = ?'; params.push(filters.category_id) }
    if (filters.account_id)  { sql += ' AND (t.account_id = ? OR t.transfer_account_id = ?)'
  ...
  getOldTxForBalance(id) {
    return this.get(
      'SELECT account_id, transfer_account_id, type, amount FROM transactions WHERE id = ?', id
    )
  }
}
