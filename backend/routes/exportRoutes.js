...
module.exports = function ({ apiRateLimiter, logError, requireAuth }) {
  ...
  router.get(
    ...
    `,
      ...
      // Scope to the signed-in user's own profiles — never every user's. This previously
      // ran `SELECT * FROM profiles`, so any user's export leaked ALL users' profile rows
      // (names + user_ids). Matches the worker export and the /api/profiles list route.
      const profiles = req.repos.profiles.listByUserId(req.session.userId);
