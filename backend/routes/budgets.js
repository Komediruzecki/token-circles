...
module.exports = function ({ apiRateLimiter, requireAuth }) {
  ...
  router.post(
    ...
    asyncHandler((req, res) => {
      ...
      req.repos.budgets.deleteByDateRange(
        pid,
        currStart,
  ...
  router.post(
    ...
    asyncHandler((req, res) => {
      ...
      if (existing) {
        req.repos.budgets.update(existing.id, pid, { amount });
        return res.json({
          id: existing.id,
          category_id,
          amount,
  ...
  router.get(
    ...
    asyncHandler((req, res) => {
      ...
      const forecastData = forecastMonths.map((fm) => {
        ...
        // Account for the year so a forecast crossing into next year isn't a negative diff
        // (which would zero out the inflation factor).
        const fd = new Date(fm.month + '-01');
        const nd = new Date();
        const monthsDiff =
          (fd.getFullYear() - nd.getFullYear()) * 12 + (fd.getMonth() - nd.getMonth());
