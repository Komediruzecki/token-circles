...
module.exports = function ({ db, apiRateLimiter, logError }) {
  ...
  router.get('/api/budgets', apiRateLimiter, (req, res) => {
    try {
  ...
  router.post('/api/budgets', apiRateLimiter, (req, res) => {
    try {
  ...
  router.put('/api/budgets/:id', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
      const { category_id, amount, period, start_date, end_date, rollover_enabled } = req.body;
  ...
  router.delete('/api/budgets/:id', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.put('/api/budgets/:id/rollover', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.get('/api/budgets/summary', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.post('/api/budgets/duplicate-last', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.post('/api/budgets/from-expenses', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
      const { year, month } = req.body;
  ...
  router.get('/api/budgets/history', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.get('/api/budgets/improvements', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  // BUDGET ALERTS
  // ========================
  router.get('/api/budgets/alerts', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.get('/api/budgets/zero-based', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.post('/api/budgets/allocate', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
  ...
  router.get('/api/budgets/zero-based/summary', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
      const month = req.query.month || new Date().toISOString().slice(0, 7);
      const startOfMonth = `${month}-01`;
  ...
  // BUDGET FORECASTING
  // ========================
  router.get('/api/budgets/forecast', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
