...
module.exports = function ({ db, apiRateLimiter, logError }) {
  ...
  // ========================
  router.get('/api/analytics/daily-heatmap', apiRateLimiter, asyncHandler((req, res) => {
  ...
  router.get('/api/analytics/weeks', apiRateLimiter, asyncHandler((req, res) => {
    const pids = getProfileIds(req);
    const inClause = pids.map(() => '?').join(',');
    const year = parseInt(req.query.year);
  ...
  // ========================
  router.get('/api/analytics/category-trends', apiRateLimiter, asyncHandler((req, res) => {
    const pids = getProfileIds(req);
    const inClause = pids.map(() => '?').join(',');
  ...
  // ========================
  router.get('/api/analytics/sankey', apiRateLimiter, asyncHandler((req, res) => {
    const pids = getProfileIds(req);
    const inClause = pids.map(() => '?').join(',');
    const year = parseInt(req.query.year) || new Date().getFullYear();
    const month = req.query.month ? String(req.query.month).padStart(2, '0') : null;
