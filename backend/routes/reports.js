...
module.exports = function ({
  ...
  // JSON tax summary
  router.get('/api/reports/tax-summary', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    const pids = getProfileIds(req);
    const inClause = pids.map(() => '?').join(',');
  ...
  router.get('/api/reports/tax-summary-pdf', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    const { year } = req.query;
  ...
  // JSON P&L summary
  router.get('/api/reports/pl-summary', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    const pids = getProfileIds(req);
    const inClause = pids.map(() => '?').join(',');
    const { year } = req.query;
    if (!year) return res.status(400).json({ error: 'year is required' });
  ...
  // Year-end P&L PDF
  router.get('/api/reports/pl-summary-pdf', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    const { year } = req.query;
    if (!year || !/^\d{4}$/.test(String(year))) {
      return res.status(400).json({ error: 'Valid year is required' });
    }
  ...
  router.post('/api/reports/custom', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
  ...
  // ── Overview Report ─────────────────────────────────────────────────
  router.get('/api/reports/overview', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
  ...
  router.get('/api/reports/custom/:id', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
  ...
  router.put('/api/reports/custom/:id', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    const id = parseInt(req.params.id);
    const report = customReports.get(id);
    if (!report) return res.status(404).json({ error: 'Report not found' });
  ...
  router.delete('/api/reports/custom/:id', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    const id = parseInt(req.params.id);
  ...
  // ── Report Comparison ────────────────────────────────────────────────
  router.get('/api/reports/compare', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    const pid = getProfileId(req);
  ...
  // ── Saved Reports ────────────────────────────────────────────────────
  router.get('/api/reports/saved', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
    res.json({ reports: [] });

  }));

  router.post('/api/reports/save', apiRateLimiter, requireAuth, asyncHandler((req, res) => {
