...
module.exports = function({ db, apiRateLimiter, uploadReceipt, logError }) {
  ...
  router.post('/api/receipts/upload', apiRateLimiter, uploadReceipt.single('receipt'), (req, res) => {
    try {
      ...
      const stmt = db.prepare(`INSERT INTO receipts (transaction_id, filename, original_name, file_type, file_size, storage_path, profile_id) VALUES (?, ?, ?, ?, ?, ?, ?)`);
      const result = stmt.run(transaction_id, filename, req.file.originalname, fileType, stats.size, storagePath, pid);
      res.json({ id: result.lastInsertRowid, transaction_id: parseInt(transaction_id), filename, original_name: req.file.originalname, file_type: fileType, file_size: stats.size, url: `/receipts/${filename}`, uploaded_at: new Date().toISOString() });
    } catch (err) {
      console.error(err.message);
      logError('error', err);
      if (err.code === 'ENOENT' || err.code === 'EACCES') { res.status(500).json({ error: 'Upload directory not accessible' }); }
      else { res.status(500).json({ error: 'Upload failed. Please try again.' }); }
    }
  });
  ...
  router.get('/api/receipts/:id', apiRateLimiter, (req, res) => {
    try {
      ...
      res.json(receipt);
    } catch (err) { console.error(err.message); logError('error', err); res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/api/receipts/transaction/:transactionId', apiRateLimiter, (req, res) => {
    try {
      ...
      const receipt = db.prepare('SELECT * FROM receipts WHERE transaction_id = ? AND profile_id = ?').get(transactionId, pid);
      if (!receipt) return res.status(404).json({ error: 'Receipt not found' });
      res.json(receipt);
    } catch (err) { console.error(err.message); logError('error', err); res.status(500).json({ error: err.message }); }
  });
  ...
  router.get('/api/receipts/file/:filename', apiRateLimiter, (req, res) => {
    try {
      ...
      res.sendFile(storagePath, {}, (err) => { if (err && err.code !== 'ECONNABORTED') console.error('Error sending file:', err); });
    } catch (err) { console.error(err.message); logError('error', err); res.status(500).json({ error: err.message }); }
  });
  ...
  router.delete('/api/receipts/:id', apiRateLimiter, (req, res) => {
    try {
      ...
      if (!receipt) return res.status(404).json({ error: 'Receipt not found' });
      try { if (fs.existsSync(receipt.storage_path)) fs.unlinkSync(receipt.storage_path); } catch (err) { console.error('Error deleting receipt file:', err); }
