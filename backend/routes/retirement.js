...
module.exports = function ({ apiRateLimiter, logError, requireAuth }) {
  ...
  // ── Saved planner assumptions ────────────────────────────────────────────────
  // The planner (frontend/src/features/RetirementPlanner.tsx) asks for these on every
  // visit, unconditionally. They are a Worker feature and this server had no route for
  // them, so every visit to the retirement page against this backend was a guaranteed
  // 404 — a console error on a page that otherwise works, which page-loading.spec.ts
  // catches whenever its sampling window lands on the request.
  //
  // Deliberately storing NO defaults: an empty object is returned when nothing has been
  ...
  router.get(
    ...
    asyncHandler((req, res) => {
      const pid = getProfileId(req);
      const row = req.repos.settings.get(settingsKeyFor(pid));
      let settings = {};
      if (row && row.value) {
        try {
          settings = JSON.parse(row.value);
        } catch {
          // A blob this server cannot parse is not worth failing the page for; the client
          // normalises whatever it gets, and an empty object is the honest answer.
          settings = {};
        }
      }
      // `filled` and `missing` describe values derived from the user's own data, which this
      // server does not compute. Empty is accurate here, not a placeholder.
      res.json({ settings, filled: [], missing: [], startMonth: currentMonth() });
    })
  );
