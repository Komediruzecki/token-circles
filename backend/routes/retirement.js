...
module.exports = function ({ db, apiRateLimiter, logError }) {
  ...
  router.get('/api/retirement-goals', apiRateLimiter, (req, res) => {
    try {
  ...
  router.post('/api/retirement-goals', apiRateLimiter, (req, res) => {
    try {
  ...
  router.put('/api/retirement-goals/:id', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
      const {
        name,
  ...
  router.delete('/api/retirement-goals/:id', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
      const result = db
  ...
  router.post('/api/calculator/retire', apiRateLimiter, (req, res) => {
    try {
      const {
        currentAge = 30,
        retirementAge = 65,
  ...
  router.get('/api/retirement/projection', apiRateLimiter, (req, res) => {
    try {
      const pid = getProfileId(req);
