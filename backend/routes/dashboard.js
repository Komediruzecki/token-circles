...
module.exports = function ({ db, apiRateLimiter, logError }) {
  ...
  // ========================
  router.get('/api/dashboard/summary', apiRateLimiter, (req, res) => {
    try {
      const pids = getProfileIds(req);
      const inClause = pids.map(() => '?').join(',');
  ...
  router.get('/api/dashboard/charts', apiRateLimiter, (req, res) => {
    try {
      const pids = getProfileIds(req);
      const inClause = pids.map(() => '?').join(',');
  ...
  router.get('/api/dashboard/net-worth', apiRateLimiter, (req, res) => {
    try {
      const pids = getProfileIds(req);
      const inClause = pids.map(() => '?').join(',');
      // Get account balances
