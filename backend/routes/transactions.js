...
module.exports = function ({ apiRateLimiter, requireAuth, logError }) {
  ...
  router.post(
    ...
    apiRateLimiter,
    requireAuth,
    validate(transactionCreateSchema, 'body'),
    asyncHandler((req, res) => {
      ...
      // Persist the transaction row and its account-balance side effects atomically:
      // a mid-sequence failure must not leave a committed row with half-applied balances.
      const info = req.repos.transactions.db.transaction(() => {
    ...
    asyncHandler((req, res) => {
      ...
      if (action === 'delete' || action === 'DELETE' || action.toLowerCase() === 'delete') {
        ...
        const result = req.repos.transactions.db.transaction(() => {
          ...
          return req.repos.transactions.run(
  ...
  router.put(
    ...
    apiRateLimiter,
    requireAuth,
    validate(transactionUpdateSchema, 'body'),
    asyncHandler((req, res) => {
      const pid = getProfileId(req);
      const pids = getProfileIds(req);
      const inClause = pids.map(() => '?').join(',');
      ...
      // Reverse the old effect, write the row update, then apply the new effect — all in one
      // transaction. If the row does not belong to this profile the UPDATE changes nothing, so
      // the whole unit rolls back (undoing the reversal too) and we report 404. This replaces the
      // previous manual "re-apply on 0 changes" compensation, which was itself non-atomic.
      updates.push("updated_at=datetime('now')");
      ...
      const NOT_FOUND = {};
      try {
        req.repos.transactions.db.transaction(() => {
          ...
          if (result.changes === 0) throw NOT_FOUND;
  ...
  router.delete(
    ...
    asyncHandler((req, res) => {
      ...
      // Delete the row and reverse its balance effect atomically; a non-matching id rolls back to 404.
      const NOT_FOUND = {};
      try {
        req.repos.transactions.db.transaction(() => {
          const result = req.repos.transactions.run(
  ...
  router.post(
    '/api/transactions/reconcile/bulk',
    apiRateLimiter,
    requireAuth,
    asyncHandler((req, res) => {
      const pid = getProfileId(req);
  ...
  router.put(
    '/api/transactions/reconcile-batch',
    apiRateLimiter,
    requireAuth,
    asyncHandler((req, res) => {
      const pid = getProfileId(req);
