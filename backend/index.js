...
const { toCamelCase, getCategoryIcon, calculateRetirementProjection } = require('./utils');
const { getProfileId, getProfileIds, profileWhere, profileInClause, profileParams } = require('./middleware/profile');
...
// Get all portfolio holdings + current prices
app.get('/api/portfolio/holdings', apiRateLimiter, async (req, res) => {
  try {
    const pids = getProfileIds(req);
    const inClause = pids.map(() => '?').join(',');
    const holdings = db
...
app.get('/api/portfolio/summary', apiRateLimiter, async (req, res) => {
  try {
    const pids = getProfileIds(req);
    const inClause = pids.map(() => '?').join(',');
    const holdings = db
...
// Add a new holding
app.post('/api/portfolio/holdings', apiRateLimiter, (req, res) => {
  try {
    const pid = getProfileId(req);
    const { ticker, shares, purchase_price, purchase_date, notes } = req.body;

    if (!ticker || !shares || !purchase_price || !purchase_date) {
      return res.status(400).json({ error: 'ticker, shares, purchase_price, and purchase_date are required' });
    }
    ...
    const result = db
      ...
      .run(String(ticker).toUpperCase(), parseFloat(shares), parseFloat(purchase_price), purchase_date, notes || '', pid);
...
// Update a holding
app.put('/api/portfolio/holdings/:id', apiRateLimiter, (req, res) => {
  try {
    const pid = getProfileId(req);
    const holdingId = parseInt(req.params.id);
    const { ticker, shares, purchase_price, purchase_date, notes } = req.body;

    const existing = db.prepare('SELECT * FROM portfolio_holdings WHERE id = ? AND profile_id = ?').get(holdingId, pid);
    if (!existing) {
...
// Delete a holding
app.delete('/api/portfolio/holdings/:id', apiRateLimiter, (req, res) => {
  try {
    const pid = getProfileId(req);
    const holdingId = parseInt(req.params.id);
    ...
    db.prepare('DELETE FROM portfolio_holdings WHERE id = ? AND profile_id = ?').run(holdingId, pid);
    res.json({ ok: true });
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ error: err.message });
...
app.post('/api/portfolio/prices', apiRateLimiter, async (req, res) => {
  try {
  ...
  try {
    ...
    if (data.portfolioHoldings && data.portfolioHoldings.length > 0) {
      ...
      for (const h of data.portfolioHoldings) {
        insertHolding.run(
          h.ticker, h.shares, h.purchase_price || h.purchasePrice,
          h.purchase_date || h.purchaseDate, h.notes || '', pid
        );
      }
    }
