...
- Bills and Housing now share `ToggleField` for Autopay: an unframed settings row with a compact orbital title accent, stable subtitle/switch columns, and `aria-labelledby`/`aria-describedby` wiring. This removes the redundant refresh icon, stacked separator lines, and narrow-modal text collisions.
