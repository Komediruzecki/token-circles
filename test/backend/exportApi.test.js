  ...
  describe('GET /api/export/:type', () => {
    ...
    describe.each(validTypes)('type: %s', (type) => {
      test(`returns 200 for ${type} export`, async () => {
        const resp = await request(BASE_URL).get(`/api/export/${type}`);
      ...
      test(`returns CSV with format=csv for ${type}`, async () => {
        const resp = await request(BASE_URL).get(`/api/export/${type}?format=csv`);
        expect(resp.status).toBe(200);
        expect(resp.headers['content-type']).toContain('text/csv');
      });

      test(`returns JSON with format=json for ${type}`, async () => {
        const resp = await request(BASE_URL).get(`/api/export/${type}?format=json`);
        expect(resp.status).toBe(200);
    ...
    test('returns 400 for invalid export type', async () => {
      const resp = await request(BASE_URL).get('/api/export/invalid-type');
    ...
    test('returns 200 for /api/export/ (serves index.html due to static middleware)', async () => {
      const resp = await request(BASE_URL).get('/api/export/');
    ...
    test('transactions export includes expected columns', async () => {
      const resp = await request(BASE_URL).get('/api/export/transactions?format=json');
    ...
    test('categories export includes expected columns', async () => {
      const resp = await request(BASE_URL).get('/api/export/categories?format=json');
      const data = JSON.parse(resp.text);
      if (data.length > 0) {
        const first = data[0];
    ...
    test('accounts export includes expected columns', async () => {
      const resp = await request(BASE_URL).get('/api/export/accounts?format=json');
      const data = JSON.parse(resp.text);
      if (data.length > 0) {
        const first = data[0];
        expect(first).toHaveProperty('name');
    ...
    test('loans export includes expected columns', async () => {
      const resp = await request(BASE_URL).get('/api/export/loans?format=json');
      const data = JSON.parse(resp.text);
      if (data.length > 0) {
        const first = data[0];
        expect(first).toHaveProperty('name');
    ...
    test('recurring export includes expected columns', async () => {
      const resp = await request(BASE_URL).get('/api/export/recurring?format=json');
      const data = JSON.parse(resp.text);
      if (data.length > 0) {
        const first = data[0];
        expect(first).toHaveProperty('description');
