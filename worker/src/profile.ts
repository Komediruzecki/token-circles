...
export async function getProfileId(c: Context<AppEnv>): Promise<number> {
  ...
  // Reject a malformed header (e.g. "12abc") rather than let parseInt salvage a number;
  // fall back to the user's first profile. (Ownership is verified below regardless.)
...
export async function getProfileIds(c: Context<AppEnv>): Promise<number[]> {
  ...
  for (const id of ids)
    if (!owned.has(id)) throw new HttpError(403, `Access denied to profile ${id}`);
  return ids;
}
