...
calculatorsRoutes.get('/api/calculators/loans', requireAuth, async (c) => {
  ...
  if (isNaN(principal) || principal <= 0) throw new HttpError(400, 'Principal must be a positive number')
  if (isNaN(rate) || rate < 0) throw new HttpError(400, 'Rate must be a non-negative number')
  if (isNaN(term) || term < 1 || !Number.isInteger(term)) throw new HttpError(400, 'Term must be a positive integer')
...
calculatorsRoutes.get('/api/calculators/mortgages', requireAuth, async (c) => {
  ...
  const totalInterest =
    totalPayment - loanAmount - propertyTax * term - monthlyInsurance * termMonths - pmi * termMonths
...
calculatorsRoutes.get('/api/calculators/savings', requireAuth, async (c) => {
  ...
  if (isNaN(goalAmount) || goalAmount <= 0) throw new HttpError(400, 'Goal amount must be positive')
  ...
  if (monthlyContribution < 0) throw new HttpError(400, 'Monthly contribution must be non-negative')
  if (isNaN(annualRate) || annualRate < 0) throw new HttpError(400, 'Annual rate must be non-negative')
...
calculatorsRoutes.get('/api/calculators/retirement', requireAuth, async (c) => {
  ...
  if (monthlyContribution < 0) throw new HttpError(400, 'Monthly contribution must be non-negative')
  if (retirementAge <= currentAge) throw new HttpError(400, 'Retirement age must be greater than current age')
  if (!['US', 'CA', 'GB', 'AU', 'DE', 'FR'].includes(country)) throw new HttpError(400, 'Unsupported country code')
...
calculatorsRoutes.get('/api/calculator/emergency-fund', requireAuth, async (c) => {
  ...
  const avgMonthlyExpenses =
    monthsWithData > 0 ? Object.values(monthlyTotals).reduce((a, b) => a + b, 0) / monthsWithData : 0
  ...
  const totalEmergencyFund = accounts.filter((a) => a.type === 'savings').reduce((s, a) => s + a.balance, 0)
