...
tagsRoutes.post('/api/tags', requireAuth, async (c) => {
  ...
  const dupe = await db.first(c.env.DB, 'SELECT id FROM tags WHERE name = ? AND profile_id = ?', name, pid)
  if (dupe) throw new HttpError(400, 'Tag already exists')
  let color = b.color
  if (!color) {
    const row = await db.first<{ c: number }>(c.env.DB, 'SELECT COUNT(*) AS c FROM tags WHERE profile_id = ?', pid)
...
tagsRoutes.get('/api/transactions/:id/tags', requireAuth, async (c) => {
  const pid = await getProfileId(c)
  const tx = await db.first(c.env.DB, 'SELECT id FROM transactions WHERE id = ? AND profile_id = ?', c.req.param('id'), pid)
  if (!tx) throw new HttpError(404, 'Transaction not found')
...
tagsRoutes.get('/api/transactions/by-tag/:tagId', requireAuth, async (c) => {
  ...
  if (categoryIds) {
    ...
    if (ids.length) { sql += ` AND t.category_id IN (${ids.map(() => '?').join(',')})`; params.push(...ids) }
  }
