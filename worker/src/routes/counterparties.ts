...
counterpartiesRoutes.get('/api/counterparties', requireAuth, async (c) => {
  ...
  const outgoing = await db.all<{ name: string; total: number; count: number }>(
    c.env.DB,
    `SELECT beneficiary AS name, SUM(amount) AS total, COUNT(*) AS count
     FROM transactions
  ...
  const incoming = await db.all<{ name: string; total: number; count: number }>(
    c.env.DB,
    `SELECT payor AS name, SUM(amount) AS total, COUNT(*) AS count
     FROM transactions
  ...
  const map = new Map<string, { name: string; incoming: number; outgoing: number; count: number }>()
