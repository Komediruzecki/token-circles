import type { ParseDeps } from './xlsx';
...
export interface TransferRuleSet {
   ...
   * E.g. { '1111': 'Erste Current', 'ibkr': 'IB' }.
   */
...
export interface BankAdapter {
   ...
   * `deps` carries capabilities the runtime must supply (currently only an
   * xlsx reader, for adapters whose statements are spreadsheets); adapters
   * parsing plain text ignore it.
   */
  parse(bytes: Uint8Array, filename: string, deps?: ParseDeps): Promise<ParsedStatement>;
