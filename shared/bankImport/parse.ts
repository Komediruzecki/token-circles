 ...
 * (except `parseXls`, which takes an injected xlsx reader) and unit-tested.
 */
import type { XlsxLoader } from './xlsx';
...
export function decodeText(bytes: Uint8Array, encoding = 'utf-8'): string {
  // Both flags spelled out: workers-types declares them required, lib.dom optional, and
  // this file compiles under both. `false` is the default in each, so behaviour is unchanged
  // (ignoreBOM: false means a leading BOM is stripped rather than kept as a character).
 ...
 */
export async function parseXls(bytes: Uint8Array, loadXlsx?: XlsxLoader): Promise<string[][]> {
  if (!loadXlsx) {
    throw new Error('This statement is a spreadsheet, which this importer cannot read here');
  }
  const XLSX = await loadXlsx();
  const wb = XLSX.read(bytes, { type: 'array', cellDates: true });
  const first = wb.SheetNames[0];
  if (!first) return [];
  const sheet = wb.Sheets[first] as never;
 ...
 * Used for Erste ("2.468,13", "100,00", "-1,59").
 */
