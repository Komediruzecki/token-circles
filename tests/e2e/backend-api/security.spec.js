...
describe('Security E2E', () => {
  ...
  describe('Rate Limiting', () => {
    ...
    test('SEC-003: Test reset rate limit endpoint', async () => {
      const resp = await agent.post('/api/test/reset-rate-limit');
      expect(resp.status).toBe(200);
    });
  });
  ...
  describe('Security Headers', () => {
    ...
    test('SEC-008: Response includes Referrer-Policy header', async () => {
      const resp = await agent.get('/api/health');
      expect(resp.headers['referrer-policy']).toBeDefined();
    });
  });
  ...
  describe('CSRF Protection', () => {
    test('SEC-009: POST requests require valid session', async () => {
      ...
      const resp = await agentNoSession.post('/api/categories').send({
        name: 'Test Category'
      });
    ...
    test('SEC-010: PUT requests require valid session', async () => {
      ...
      const agentNoSession = request(BASE_URL);
      const resp = await agentNoSession.put('/api/categories/1').send({
        name: 'Updated'
      });
    ...
    test('SEC-011: DELETE requests require valid session', async () => {
      const agentNoSession = request(BASE_URL);
      const resp = await agentNoSession.delete('/api/categories/1');
  ...
  describe('Authentication Validation', () => {
    test('SEC-012: Reject requests without authentication', async () => {
      ...
      const resp1 = await agentNoSession.get('/api/transactions');
      expect(resp1.status).toBe(401);

      const resp2 = await agentNoSession.post('/api/categories').send({ name: 'Test' });
      expect(resp2.status).toBe(401);

      const resp3 = await agentNoSession.delete('/api/categories/1');
  ...
  describe('Sensitive Data Handling', () => {
    ...
    test('SEC-015: API errors don't leak sensitive info', async () => {
      ...
      expect(resp.body).not.toHaveProperty('stack');
    });
  });
  ...
  describe('HTTPS Headers', () => {
    test('SEC-020: HSTS header present (if using HTTPS)', async () => {
      ...
      if (resp.headers['strict-transport-security']) {
        expect(resp.headers['strict-transport-security']).toMatch(/max-age/);
      }
    });
  ...
  describe('Audit Logging', () => {
    test('SEC-022: Failed auth attempts logged', async () => {
      ...
      const resp = await agent.get('/api/logs');
      expect(resp.status).toBe(200);
    });
  ...
  describe('Data Integrity', () => {
    ...
    test('SEC-025: PUT/POST use correct HTTP methods', async () => {
      const resp = await agent.put('/api/categories/1').send({
        name: 'Updated'
      });
      expect(resp.status).toBe(200);
    });
    ...
    test('SEC-026: DELETE use correct HTTP method', async () => {
      const resp = await agent.delete('/api/categories/1');
      expect(resp.status).toBe(404); // Category doesn't exist, not method error
    });
  });
  ...
  describe('Error Messages', () => {
    test('SEC-027: Generic error messages for unauthorized', async () => {
      const agentNoSession = request(BASE_URL);
      const resp = await agentNoSession.get('/api/transactions');
      expect(resp.status).toBe(401);
    ...
    test('SEC-029: No sensitive data in API response', async () => {
      ...
      if (resp.body.rows) {
        resp.body.rows.forEach(tx => {
          expect(tx).not.toHaveProperty('password');
        });
      }
    });
