...
describe('Accounts E2E', () => {
  ...
  afterAll(async () => {
    ...
    if (testAccountId3) await agent.delete(`/api/accounts/${testAccountId3}`).set('X-Skip-RateLimit', 'true').catch(() => {});
  ...
  describe('Account Creation', () => {
    test('BE-ACC-001: Create basic account', async () => {
      ...
      global.expect(resp.body.type).toBe('checking');
    ...
    test('BE-ACC-002: Create savings account', async () => {
      ...
      global.expect(resp.status).toBe(200);
      global.expect(resp.body.type).toBe('savings');
    ...
    test('BE-ACC-003: Create credit card account', async () => {
      ...
      global.expect(resp.status).toBe(200);
      global.expect(resp.body.type).toBe('credit');
      global.expect(resp.body.balance).toBeLessThan(0);
    ...
    test('BE-ACC-005: Account initial balance tracks correctly', async () => {
      ...
      global.expect(resp.status).toBe(200);
      global.expect(resp.body.initialBalance).toBe(750.50);
    });
  ...
  describe('Account Retrieval', () => {
    ...
    test('BE-ACC-010: Accounts include all expected fields', async () => {
      ...
      global.expect(resp.body).toHaveProperty('initialBalance');
    ...
    test('BE-ACC-011: Accounts sorted by balance', async () => {
      ...
      if (resp.body.length > 1) {
        ...
        global.expect(balances[0]).toBeLessThanOrEqual(balances[1]);
      }
    });
  });
  ...
  describe('Account Deletion', () => {
    ...
    test('BE-ACC-017: Delete account with transactions', async () => {
      const accResp = await agent.post('/api/accounts').set('X-Skip-RateLimit', 'true').send({
      ...
      const deleteResp = await agent.delete(`/api/accounts/${accId}`).set('X-Skip-RateLimit', 'true');
      global.expect(deleteResp.status).toBe(200);

      const txCheck = await agent.get(`/api/transactions/${testTxId}`).set('X-Skip-RateLimit', 'true');
      global.expect(txCheck.body.accountId).toBeNull();
    });
  ...
  describe('Balance Tracking', () => {
    ...
    test('BE-ACC-020: Balance tracks negative accounts (credit)', async () => {
      ...
      const accResp = await agent.get(`/api/accounts/${testAccountId3}`).set('X-Skip-RateLimit', 'true');
      global.expect(accResp.status).toBe(200);
      global.expect(accResp.body.balance).toBeLessThan(0);
    });
    ...
    test('BE-ACC-021: Balance tracks positive accounts (checking)', async () => {
      ...
      const accResp = await agent.get(`/api/accounts/${testAccountId}`).set('X-Skip-RateLimit', 'true');
      global.expect(accResp.status).toBe(200);
      global.expect(accResp.body.balance).toBeGreaterThanOrEqual(0);
    });
  });
  ...
  describe('Multiple Accounts', () => {
    ...
    test('BE-ACC-023: Each account has separate balance', async () => {
      ...
      if (accounts.body.length >= 2) {
        ...
        global.expect(acc1.id).not.toBe(acc2.id);
        global.expect(acc1.balance).not.toBe(acc2.balance);
      }
    });
    ...
    test('BE-ACC-024: Transactions assigned to correct account', async () => {
      ...
      if (accounts.body.length >= 2) {
        ...
        const tx1 = await agent.post('/api/transactions').set('X-Skip-RateLimit', 'true').send({
        ...
        const tx2 = await agent.post('/api/transactions').set('X-Skip-RateLimit', 'true').send({
        ...
        const acc2Resp = await agent.get(`/api/accounts/${acc2.id}`).set('X-Skip-RateLimit', 'true');

        global.expect(acc1Resp.body.balance).toBeCloseTo(-50, 2);
        global.expect(acc2Resp.body.balance).toBeCloseTo(-75, 2);
      }
    });
  });
  ...
  describe('Account Types', () => {
    ...
    test('BE-ACC-031: Support for investment account type', async () => {
      ...
      global.expect(resp.status).toBe(200);
      global.expect(resp.body.type).toBe('investment');
    });
  });
    ...
    test('BE-ACC-036: Accounts display correct type icon', async () => {
      ...
      if (resp.body.length > 0) {
        resp.body.forEach(acc => {
          global.expect(['checking', 'savings', 'credit', 'investment']).toContain(acc.type);
        });
      }
    });
