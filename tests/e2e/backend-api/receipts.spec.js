...
describe('Receipts E2E', () => {
  ...
  describe('Receipt Upload', () => {
    ...
    test('BE-RCP-005: Reject file with invalid extension', async () => {
      ...
      const resp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', txtContent, 'receipt.pdf');
      global.expect([400, 422]).to.include(resp.status);
    });
  });
  ...
  describe('Receipt OCR Parsing', () => {
    test('BE-RCP-006: OCR extracts total amount', async () => {
      ...
      const resp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', receiptContent, 'receipt.jpg');
      if (resp.status === 200) {
        global.expect(resp.body).toHaveProperty('totalAmount');
        global.expect(resp.body.totalAmount).toBeFinite();
      }
    });
    ...
    test('BE-RCP-007: OCR extracts date', async () => {
      ...
      const resp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', receiptContent, 'receipt.jpg');
      if (resp.status === 200) {
        global.expect(resp.body).toHaveProperty('date');
      }
    });
    ...
    test('BE-RCP-008: OCR extracts merchant name', async () => {
      ...
      const resp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', receiptContent, 'receipt.jpg');
      if (resp.status === 200) {
        global.expect(resp.body).toHaveProperty('merchant');
      }
    });
    ...
    test('BE-RCP-009: Extract line items from receipt', async () => {
      ...
      const resp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', receiptContent, 'receipt.jpg');
      if (resp.status === 200) {
        global.expect(resp.body).toHaveProperty('items');
        global.expect(Array.isArray(resp.body.items)).toBe(true);
      }
    });
  });
  ...
  describe('Receipt Line Items', () => {
    test('BE-RCP-010: Parse individual line items', async () => {
      ...
      if (resp.status === 200 && resp.body.items) {
        global.expect(resp.body.items.length).toBeGreaterThan(0);
        resp.body.items.forEach(item => {
          global.expect(item).toHaveProperty('name');
          global.expect(item).toHaveProperty('amount');
        });
      }
    });
    ...
    test('BE-RCP-011: Line items sum to total', async () => {
      ...
      if (resp.status === 200) {
        ...
        global.expect(actualTotal).toBeCloseTo(expectedTotal, 2);
      }
    });
    ...
    test('BE-RCP-012: Handle multiple decimal places in items', async () => {
      ...
      const resp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', receiptContent, 'receipt.jpg');
      if (resp.status === 200) {
        global.expect(resp.body.totalAmount).toBeDefined();
      }
    });
  ...
  describe('Receipt Split & Share', () => {
    test('BE-RCP-014: Split receipt among multiple people', async () => {
      ...
      if (resp.status === 200) {
        // Share receipt
        const shareResp = await agent.post(`/api/receipts/${resp.body.id}/share`).set('X-Skip-RateLimit', 'true').send({
    ...
    test('BE-RCP-016: Split by amount', async () => {
      ...
      if (resp.status === 200) {
        ...
        const splitResp = await agent.post(`/api/receipts/${resp.body.id}/split`).set('X-Skip-RateLimit', 'true').send({
          by: 'amount',
          amount: 30
        });
  ...
  describe('Receipt Export', () => {
    test('BE-RCP-018: Export receipt as PDF', async () => {
      ...
      if (uploadResp.status === 200) {
        const exportResp = await agent.get(`/api/receipts/${uploadResp.body.id}/export`).set('X-Skip-RateLimit', 'true').query({
          format: 'pdf'
        });
        global.expect(exportResp.status).toBe(200);
        global.expect(exportResp.headers['content-type']).toMatch(/pdf/);
      }
    });
    ...
    test('BE-RCP-019: Export receipt as CSV', async () => {
      ...
      if (uploadResp.status === 200) {
        ...
        global.expect(exportResp.status).toBe(200);
        global.expect(exportResp.headers['content-type']).toMatch(/csv/);
      }
    });
    ...
    test('BE-RCP-020: Export receipt as JSON', async () => {
      ...
      if (uploadResp.status === 200) {
        ...
        global.expect(exportResp.status).toBe(200);
        global.expect(exportResp.headers['content-type']).toMatch(/json/);
      }
    });
  });
  ...
  describe('Receipt Management', () => {
    ...
    test('BE-RCP-022: Get single receipt by ID', async () => {
      const uploadResp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', Buffer.from('Receipt Data'), 'receipt.jpg');
      if (uploadResp.status === 200) {
        const receiptId = uploadResp.body.id;
        const getResp = await agent.get(`/api/receipts/${receiptId}`).set('X-Skip-RateLimit', 'true');
        global.expect(getResp.status).toBe(200);
        global.expect(getResp.body.id).toBe(receiptId);
      }
    });
    ...
    test('BE-RCP-023: Update receipt metadata', async () => {
      const uploadResp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', Buffer.from('Receipt Data'), 'receipt.jpg');
      if (uploadResp.status === 200) {
        const updateResp = await agent.put(`/api/receipts/${uploadResp.body.id}`).set('X-Skip-RateLimit', 'true').send({
          note: 'Updated note'
        });
    ...
    test('BE-RCP-025: Get receipts by date range', async () => {
      const resp = await agent.get('/api/receipts').set('X-Skip-RateLimit', 'true').query({
  ...
  describe('Receipt Statistics', () => {
    ...
    test('BE-RCP-030: Monthly receipt spending', async () => {
      ...
      if (resp.body.length > 0) {
        ...
        global.expect(Object.keys(monthly).length).toBeGreaterThan(0);
      }
    });
  });
  ...
  describe('Receipt Organization', () => {
    ...
    test('BE-RCP-032: Assign category to receipt', async () => {
      ...
      const uploadResp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', receiptContent, 'receipt.jpg');
      if (uploadResp.status === 200) {
        const catResp = await agent.put(`/api/receipts/${uploadResp.body.id}`).set('X-Skip-RateLimit', 'true').send({
          categoryId: 1
        });
        global.expect(catResp.status).toBe(200);
      }
    ...
    test('BE-RCP-033: Add note to receipt', async () => {
      ...
      const uploadResp = await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', receiptContent, 'receipt.jpg');
      if (uploadResp.status === 200) {
        const noteResp = await agent.put(`/api/receipts/${uploadResp.body.id}`).set('X-Skip-RateLimit', 'true').send({
          note: 'Weekly grocery shopping'
        });
        global.expect(noteResp.status).toBe(200);
      }
    });
  });
  ...
  describe('Receipt Performance', () => {
    test('BE-RCP-034: Handle many receipts efficiently', async () => {
      for (let i = 0; i < 20; i++) {
        await agent.post('/api/receipts').set('X-Skip-RateLimit', 'true').attach('receipt', Buffer.from(`Receipt ${i}`), `receipt${i}.jpg`);
      }

      const resp = await agent.get('/api/receipts').set('X-Skip-RateLimit', 'true');
      global.expect(resp.status).toBe(200);
      global.expect(resp.body.length).toBeGreaterThanOrEqual(20);
    });
