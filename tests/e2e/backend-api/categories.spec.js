...
describe('Categories E2E', () => {
  ...
  describe('Category Creation', () => {
    ...
    test('BE-CAT-003: Auto-generate color from name', async () => {
      ...
      global.expect(resp.status).toBe(200);
      global.expect(resp.body).toHaveProperty('color');
      // Color should be a valid 6-digit hex code (the backend defaults to #6b7280)
  ...
  describe('Category Updates', () => {
    test('BE-CAT-011: Update category name', async () => {
      ...
      await agent.post('/api/categories').set('X-Skip-RateLimit', 'true').send({ name: oldName });
      const createResp = await agent.get('/api/categories').set('X-Skip-RateLimit', 'true').query({ limit: 1 });
  ...
  describe('Category Deletion', () => {
    ...
    test('BE-CAT-015: Delete category with assigned transactions', async () => {
      const catResp = await agent.post('/api/categories').set('X-Skip-RateLimit', 'true').send({ name: 'CatWithTx_' + Date.now() });
      ...
      await agent.put(`/api/transactions/${testTxId}`).set('X-Skip-RateLimit', 'true').send({ categoryId: catId });

      const deleteResp = await agent.delete(`/api/categories/${catId}`).set('X-Skip-RateLimit', 'true');
      ...
      global.expect(txCheck.body.categoryId).toBeNull();
    });
