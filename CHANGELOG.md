...
- **The period bar is two rows on a phone, not three.** The six quick-period buttons wrapped onto two lines and left "All" stranded on a third by itself, stretched across the whole width. "All" now sits beside the back and forward arrows, where there was already room, and the other five share the row below.
...
- **A category with a budget set no longer looks like it has a line drawn through it twice.** The budget sat under the category in four stacked rows, and the last of them drew its own horizontal rule immediately beneath the progress bar — two lines a few pixels apart, which read as one divider painted twice. Those four rows are now two: the limit sits next to the amount you have spent ("254.70 of 220.00"), and what is left of it sits next to the bar. Same numbers, three fewer lines, and a category with a budget no longer towers over one without.
- **The Allocate Budget form was unreadable in dark mode.** The category dropdown showed white text on a white list in Chrome, and the amount box looked like a plain unstyled field, because every control in that form was being handed a style rule that did not exist. They now look like the rest of the app's forms.
