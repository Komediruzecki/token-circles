...
- Name: accounts-crud.spec.ts >> Accounts CRUD Operations >> should have account type badge text
- Location: tests/accounts-crud.spec.ts:218:3

# Error details

```
Error: locator.textContent: Error: strict mode violation: locator('.account-card .badge') resolved to 2 elements:
    1) <span class="badge badge-default">giro</span> aka getByText('giro')
    2) <span class="badge badge-success">savings</span> aka getByText('savings', { exact: true })

Call log:
  - waiting for locator('.account-card .badge')
