...
test.describe('Housing CRUD Operations', () => {
  ...
  test('should have summary cards', async ({ page }) => {
    ...
    await expect(page.locator('.housing-summary .summary-card:has-text("Monthly Total")')).toBeVisible();
    await expect(page.locator('.housing-summary .summary-card:has-text("Active Expenses")')).toBeVisible();
    await expect(page.locator('.housing-summary .summary-card:has-text("Autopay Enabled")')).toBeVisible();
  });

  test('should display monthly total', async ({ page }) => {
    await page.waitForLoadState('networkidle');

    const total = page.locator('.housing-summary .summary-card:has-text("Monthly Total") .summary-value');
  ...
  test('should display active expenses count', async ({ page }) => {
    ...
    const count = page.locator('.housing-summary .summary-card:has-text("Active Expenses") .summary-value');
  ...
  test('should display autopay count', async ({ page }) => {
    ...
    const autopayCount = page.locator('.housing-summary .summary-card:has-text("Autopay Enabled") .summary-value');
  ...
  test('should have type labels: Rent, Mortgage, HOA Fees, Property Tax, Insurance, Other', async ({ page }) => {
    await page.waitForLoadState('networkidle');
  ...
  test('should have input for property/description', async ({ page }) => {
    ...
    const modal = page.locator('.modal-overlay');
    if (await modal.isVisible({ timeout: 2000 }).catch(() => false)) {
      const descInput = modal.locator('input[placeholder*="property"], input[placeholder*="Apart"], input[placeholder*="Monthly"]');
  ...
  test('should calculate total monthly cost', async ({ page }) => {
    ...
    const hasTotal = await page.locator('.housing-summary .summary-card:has-text("Monthly Total")').isVisible({ timeout: 2000 }).catch(() => false);
