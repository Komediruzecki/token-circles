...
test.describe('Budgets CRUD Operations', () => {
  ...
  test('should show unallocated budget message', async ({ page }) => {
    const message = page.locator('.page-subtitle, .page-header p').filter({ hasText: /unallocated/i });
  ...
  test('should have forecast statistics section', async ({ page }) => {
    ...
    const hasForecast = await page.locator('.budget-forecast').isVisible({ timeout: 2000 }).catch(() => false);
    if (hasForecast) {
  ...
  test('should have modal with category name', async ({ page }) => {
    await page.locator('.modal-overlay:has-text("Allocate Budget")').click().catch(() => {});
  ...
  test('should have status badges (ok, warning, over)', async ({ page }) => {
    ...
    const hasWarning = badgeClasses.some(cls => cls.includes('badge-warning') || cls.includes('status-warning'));
    const hasOver = badgeClasses.some(cls => cls.includes('badge-over') || cls.includes('status-over'));
  ...
  test('should handle forecast toggle', async ({ page }) => {
    const toggleBtn = page.locator('.forecast-toggle-section button');
    if (await toggleBtn.isVisible().catch(() => false)) {
      const isShow = await toggleBtn.textContent()?.includes('Show');
      if (isShow) {
  ...
  test('should show toast message on successful allocation', async ({ page }) => {
    ...
    await page.locator('[data-action="budgets:allocate"]').click().catch(() => {});
  ...
  test('should have budget summary displayed correctly', async ({ page }) => {
    ...
    await expect(page.locator('.budget-summary .summary-value')).toHaveCountGreaterThanOrEqual(3);
  });
  ...
  test('should handle errors gracefully', async ({ page }) => {
    ...
    await page.locator('button').filter({ hasText: /Allocate|Add/ }).first().click();
