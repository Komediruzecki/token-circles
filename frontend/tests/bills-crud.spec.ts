...
test.describe('Bills CRUD Operations', () => {
  ...
  test('should have mark paid button', async ({ page }) => {
    ...
    const markPaidBtns = page.locator('button:has-text("Mark Paid"), button:has-text("Mark as Paid")');
  ...
  test('should have input for bill name', async ({ page }) => {
    ...
    const modal = page.locator('.modal-overlay');
    if (await modal.isVisible({ timeout: 2000 }).catch(() => false)) {
      const nameInput = modal.locator('input[type="text"], input[placeholder*="Rent"], input[placeholder*="Electricity"]');
