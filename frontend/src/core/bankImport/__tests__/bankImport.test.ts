...
describe('resolveCounterpart (longest signature wins)', () => {
  it('prefers a specific card last-4 over a generic keyword', () => {
    const rules = {
      ownAccounts: [],
      keywords: [],
      counterparts: { revolut: 'Revolut EUR', '4444': 'Revolut USD' },
    }
    // A generic "revolut" is listed first but the specific "4444" must win.
    expect(resolveCounterpart('POS Revolut**4444* Dublin', rules)).toBe('Revolut USD')
...
describe('parse utils', () => {
  it('parses number conventions', () => {
    ...
    expect(parseEuropeanNumber('2.468,13')).toBeCloseTo(2468.13)
...
describe('revolut adapter', () => {
  const csv = [
    ...
    'Topup,Current,2026-05-15 19:21:32,2026-05-15 19:21:34,Top-up by *1111,50.00,0.00,EUR,COMPLETED,212.10',
  ...
  it('emits a two-sided transfer when the counterpart resolves', async () => {
    const rules: TransferRuleSet = {
      ...
      keywords: ['top-up'],
      counterparts: { '1111': 'Erste Current' },
    }
...
describe('erste adapter', () => {
  ...
  const csv = [
    'Izvod prometa po racunu HR1234567890123456789 za period od 01.05.2026. do 31.05.2026. Valuta EUR OIB 12345678901',
    ...
    '',
    '1;01.05.2026;01.05.2026;Kaufland shop;HR99;23,49;;1.111,11;HR99;HR99;KAUFLAND;ZAGREB;111',
    '2;04.05.2026;04.05.2026;Isplata place;HR08;;2.468,13;4.321,00;HR67;HR69;ACME;ZAGREB;222',
  ].join('\n')
  ...
  it('maps debit→expense and credit→income with European amounts', async () => {
    ...
    expect(parsed.meta.iban).toBe('HR1234567890123456789')
    ...
    expect(txns[1]).toMatchObject({ type: 'Income', amount: 2468.13 })
  })
})
...
function makePbzXls(): Uint8Array {
  const aoa: unknown[][] = [
    ...
    [new Date(2026, 2, 30), 'POS placanje', 'POS Revolut**4444* Dublin', -60, ' EUR ', ''],
...
describe('pbz adapter', () => {
  it('parses the binary xls, skips metadata rows, classifies by sign', async () => {
    ...
    // POS Revolut**4444 → two-sided transfer PBZ Giro → Revolut
...
describe('account resolver', () => {
  ...
  it('matches by stored IBAN identifier, then falls back to bank name', () => {
    const ids = { 'Erste Current': ['HR1234567890123456789'] }
    expect(
      resolveTargetAccount('erste', { iban: 'HR1234567890123456789' }, 'f.csv', accounts, {}, ids)
    ).toBe('Erste Current')
