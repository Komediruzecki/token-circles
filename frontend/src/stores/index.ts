...
export function createModalStore() {
  ...
  const open = (newType: string, newData?: any) => {
    batch(() => {
      setType(newType);
...
export const { theme, toggle: toggleTheme, setTheme: setThemeValue } = (() => {
  const stores = getStores();
  return {
...
export const { isOpen: isModalOpen, type: modalType, data: modalData, open: openModal, close: closeModal } = (() => {
  const stores = getStores();
  return {
...
export const { toasts, info: toastInfo, success: toastSuccess, error: toastError } = (() => {
  const stores = getStores();
  return {
