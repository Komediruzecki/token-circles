...
const HEADER_VARIANTS: Record<string, string[]> = {
  ...
  amount_local: ['amount local', 'local amount', 'amount pln', 'amount in local currency', 'local currency', 'local curr', 'amount (local)', 'local value', 'domestic amount'],
} as const
            ...
            <button
              ...
              onClick={() => {
                const header = [
                  ...
                  'Beneficiary', 'Payor', 'Means of Payment',
                  'Exchange Rate', 'Notes', 'Type', 'Amount Local',
                ]
                const rows: string[][] = [
        ...
        <>
          ...
          <div class={styles.settingsCard} style={{ 'margin-bottom': '16px' }}>
            ...
            <p class={styles.sheetsInfo} style={{ 'margin-top': '12px', 'margin-bottom': 0 }}>
              Column names are auto-detected. Use the downloadable template below for guaranteed matching.
            </p>
          </div>
